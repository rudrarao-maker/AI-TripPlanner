    ...
    <div className="min-h-screen pb-20 animate-fade-in">
      ...
      <div className="relative bg-gradient-to-b from-emerald-500/10 to-background pt-32 pb-16 px-4">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          >
            Discover unforgettable activities, tours, and experiences curated by
            locals
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
        ...
        <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar pb-2">
          ...
          {ACTIVITY_CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${activeCategory === cat ? "bg-emerald-500 text-white shadow-md" : "bg-card border border-border/50 text-muted-foreground hover:bg-muted"}`}
            >
              {cat}
            </button>
          ))}
            ...
            >
              <Card className="glass-card group cursor-pointer overflow-hidden h-full flex flex-col">
                <div className="relative h-52 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 bg-emerald-500/90 backdrop-blur-md px-3 py-1 rounded-full text-white text-xs font-bold">
                    {item.category}
                  </div>
                ...
                <CardContent className="p-5 flex flex-col flex-1">
                  <h3 className="font-bold text-base mb-2 group-hover:text-emerald-600 transition-colors line-clamp-2">
                    {item.title}
                  </h3>
