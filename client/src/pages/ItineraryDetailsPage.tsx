...
export function ItineraryDetailsPage() {
  ...
  const handleUseItinerary = () => {
    ...
    navigate(`/plan?dest=${encodeURIComponent(itinerary.destination)}&days=${itinerary.durationDays}`);
  };
    ...
    <div className="min-h-screen pb-20 bg-muted/20">
      ...
      <div className="relative h-[50vh] min-h-[400px] w-full">
        ...
        <div className="absolute top-24 left-4 md:left-8 z-20">
          <Button variant="outline" size="icon" className="rounded-full bg-white/10 backdrop-blur-md border-white/20 text-white hover:bg-white/20 hover:text-white" onClick={() => navigate(-1)}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12 z-20 container mx-auto text-white">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="text-4xl md:text-6xl font-bold mb-4 drop-shadow-lg">{itinerary.title}</h1>
            <div className="flex flex-wrap items-center gap-6 text-sm md:text-base font-medium text-white/90">
              <div className="flex items-center gap-2"><Calendar className="h-5 w-5" /> {itinerary.durationDays} Days</div>
              <div className="flex items-center gap-2"><CreditCard className="h-5 w-5" /> Est. {itinerary.estimatedCost}</div>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 py-12 max-w-5xl flex flex-col md:flex-row gap-12">
        ...
        <div className="flex-1 space-y-12">
          <section>
            ...
            <p className="text-lg text-muted-foreground leading-relaxed">{itinerary.description}</p>
          </section>
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row gap-6 relative z-10">
                    <div className="md:w-32 shrink-0 pt-2 flex items-center md:items-start gap-4 md:gap-0">
                      ...
                      <h3 className="text-xl font-bold md:pl-6 text-primary">Day {day.day}</h3>
                    </div>
                    ...
                    <div className="flex-1 space-y-6">
                      ...
                      <div className="space-y-6">
                        {day.activities.map((activity, actIndex) => (
                          <div key={actIndex} className="bg-card border border-border p-5 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex flex-col sm:flex-row gap-5">
                              {activity.imageUrl && (
                                <img src={activity.imageUrl} alt={activity.title} className="w-full sm:w-32 h-32 object-cover rounded-lg shrink-0" />
                              )}
                              <div>
        ...
        <div className="w-full md:w-80 shrink-0">
          <div className="sticky top-24 bg-card border border-border p-6 rounded-2xl shadow-lg flex flex-col gap-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Clone this itinerary to your dashboard. You can customize the dates, add new activities, and book hotels directly.</p>
            </div>
            
            <Button size="lg" className="w-full font-bold text-lg h-14 shadow-lg hover:scale-105 transition-transform" onClick={handleUseItinerary}>
              Use This Itinerary
            </Button>
            
            <div className="text-xs text-center text-muted-foreground">
              Estimated costs are based on historical data and do not include flights.
            </div>
          </div>
        </div>
      </div>
