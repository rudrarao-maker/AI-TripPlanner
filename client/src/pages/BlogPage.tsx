    ...
    <div className="container mx-auto px-4 py-24 min-h-screen">
      ...
      >
        ...
        <div className="relative max-w-lg mx-auto mb-8">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search articles..." className="pl-12 py-5 rounded-2xl bg-card shadow-lg" />
        </div>

        {/* Category Tabs */}
        <div className="flex items-center gap-2 justify-center overflow-x-auto hide-scrollbar mb-8">
          <button onClick={() => setActiveCategory('all')} className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${activeCategory === 'all' ? 'bg-primary text-primary-foreground shadow-md' : 'bg-card border border-border/50 text-muted-foreground hover:bg-muted'}`}>
            All Posts
          </button>
          {BLOG_CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all flex items-center gap-1.5 ${activeCategory === cat.id ? 'bg-primary text-primary-foreground shadow-md' : `${cat.color} border border-border/30`}`}>
              {cat.icon} {cat.label}
            </button>
          ))}
        </div>
            ...
            <Card className="glass-card overflow-hidden group cursor-pointer mb-8">
              <div className="grid md:grid-cols-2 gap-0">
                ...
                <CardContent className="p-8 flex flex-col justify-center">
                  <span className="text-xs font-bold text-primary mb-3 uppercase tracking-wider">Featured</span>
                  ...
                  <Button variant="gradient" className="w-fit rounded-full gap-2">Read More <ChevronRight className="h-4 w-4" /></Button>
                </CardContent>
              </div>
            </Card>
          </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((post, idx) => (
            <motion.div key={post.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.05 }}>
              <Card className="glass-card group cursor-pointer overflow-hidden h-full flex flex-col hover:shadow-lg transition-all hover:-translate-y-1">
                ...
                <CardHeader className="pb-2">
                  <div className="text-sm text-primary font-bold mb-1">{post.date}</div>
