  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pt-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Manage your account security, email verification, and two-factor authentication.</p>
      </div>
      ...
      <div className="grid gap-6 mt-8">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="glass border-primary/10">
            <CardHeader>
              ...
              <CardDescription>Verify your email address to secure your account and receive travel updates.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between p-4 bg-background/50 rounded-xl border border-border/50">
                ...
                {!emailVerified && (
                  <Button variant="outline" onClick={handleVerifyEmail}>Send Link</Button>
                )}
              </div>
            </CardContent>
          </Card>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className="glass border-primary/10">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Smartphone className="h-5 w-5" /> Two-Factor Authentication (2FA)
              </CardTitle>
              ...
              {twoFactorEnabled ? (
                <div className="flex flex-col md:flex-row items-center justify-between p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
                  <div className="flex items-center gap-3 mb-4 md:mb-0">
                    ...
                    <div>
                      <p className="font-bold text-green-700 dark:text-green-400">2FA is Enabled</p>
                      <p className="text-sm text-green-600/80 dark:text-green-500/80">Your account is highly secure.</p>
                    </div>
                  </div>
                  <Button variant="destructive" onClick={() => setTwoFactorEnabled(false)}>Disable 2FA</Button>
                </div>
              ) : isSettingUp2FA ? (
                <div className="p-6 bg-background/50 rounded-xl border border-border/50 space-y-6">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">Use Google Authenticator or Authy to scan this code.</p>
                    <img 
                    ...
                    />
                    <p className="text-xs font-mono bg-muted p-2 rounded mt-4 max-w-[200px] mx-auto">JBSWY3DPEHPK3PXP</p>
                  </div>
                ...
                <div className="flex items-center justify-between p-4 bg-background/50 rounded-xl border border-border/50">
                  ...
                  <Button onClick={() => setIsSettingUp2FA(true)}>Set Up 2FA</Button>
                </div>
              )}
            </CardContent>
          </Card>
