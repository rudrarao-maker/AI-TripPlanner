          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              {upcomingTrip && (
                <Button variant="ghost" className="text-primary" asChild>
                  <Link to={`/dashboard/trips/${upcomingTrip.id}`}>View Details <ChevronRight className="h-4 w-4 ml-1" /></Link>
                </Button>
              )}
            </div>
            ...
            {upcomingTrip ? (
              <Card className="overflow-hidden border-none shadow-lg group">
                <div className="relative h-64 w-full bg-gradient-to-br from-indigo-500/20 to-purple-500/20" style={{ backgroundImage: `url(${upcomingTrip.coverImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                  ...
                  <div className="absolute top-4 right-4 z-20 bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-full text-white font-medium text-sm flex items-center gap-1.5">
                    <Clock className="h-4 w-4" /> {Math.ceil((new Date(upcomingTrip.startDate).getTime() - new Date().getTime()) / (1000 * 3600 * 24))} days left
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 z-20">
                    ...
                    <h3 className="text-3xl font-bold text-white mb-2">{upcomingTrip.title}</h3>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="col-span-2 p-4 rounded-xl bg-muted/50">
                      ...
                      <div className="flex items-end justify-between mb-2">
                        <span className="text-2xl font-bold text-foreground">{formatCurrency(0)}</span>
                        <span className="text-sm text-muted-foreground">of {formatCurrency(upcomingTrip.budget)}</span>
                      </div>
              ...
              <Card className="glass py-12 flex flex-col items-center justify-center text-center">
                <div className="h-16 w-16 bg-primary/10 rounded-full flex items-center justify-center text-3xl mb-4">🌍</div>
                <h3 className="text-xl font-bold mb-2">No trips planned yet</h3>
                <p className="text-muted-foreground mb-6">Let our AI craft the perfect itinerary for your next vacation.</p>
        ...
        <div className="space-y-8">
          ...
          <Card>
            ...
            <CardContent className="space-y-4 pt-4">
              {pastTrips.length === 0 ? (
                <div className="text-sm text-center text-muted-foreground py-4">No past trips found.</div>
              ) : pastTrips.map((trip) => (
