...
import { BookOpen, Camera, MapPin, Calendar as CalendarIcon, Download, Star } from 'lucide-react';
  ...
  const [entries, setEntries] = useState([
    {
      ...
      notes: 'The sunset was absolutely incredible. Watched the Kecak fire dance!',
      rating: 5,
    ...
    setEntries([
      {
        ...
        rating: 5,
        image: 'https://images.unsplash.com/photo-1532185987396-d8f99e3a31c5?w=800' // mock image for now
      },
      ...entries
    ]);
    ...
    <div className="container mx-auto px-4 py-8 mt-20 animate-fade-in max-w-4xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Record your memories, rate places, and export them as a beautiful PDF.</p>
        </div>
      ...
      <Card className="glass-card mb-8">
        ...
        <CardContent className="space-y-4">
          ...
          <Button variant="gradient" className="w-full sm:w-auto" onClick={handleAddEntry}>
            Save Memory
          </Button>
        </CardContent>
      </Card>
      ...
      <div id="journal-content" className="space-y-8 bg-background p-4 rounded-xl">
        {entries.map(entry => (
          <Card key={entry.id} className="overflow-hidden border-border/50">
            <div className="flex flex-col md:flex-row">
              <div className="md:w-1/3 h-48 md:h-auto relative">
                <img src={entry.image} alt={entry.location} className="absolute inset-0 w-full h-full object-cover" />
              </div>
                ...
                <div>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex text-yellow-400">
                      {[...Array(entry.rating)].map((_, i) => <Star key={i} className="h-4 w-4 fill-current" />)}
                    </div>
                  </div>
