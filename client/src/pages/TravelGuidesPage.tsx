    ...
    <div className="min-h-screen pb-20 animate-fade-in">
      ...
      <div className="relative bg-gradient-to-b from-blue-500/10 to-background pt-32 pb-16 px-4">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
            Expert tips, city guides, and insider knowledge to make your trip unforgettable
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="relative max-w-xl mx-auto">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search guides and articles..." className="pl-12 py-6 text-lg rounded-2xl bg-card border-border/50 shadow-xl" />
          </motion.div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 mt-8 space-y-12">
        ...
        <div className="flex items-center gap-3 overflow-x-auto hide-scrollbar">
          <button onClick={() => setActiveCategory('all')} className={`px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap transition-all ${activeCategory === 'all' ? 'bg-primary text-primary-foreground shadow-md' : 'bg-card border border-border/50 text-muted-foreground hover:bg-muted'}`}>
            All Guides
          </button>
          {BLOG_CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap transition-all flex items-center gap-2 ${activeCategory === cat.id ? 'bg-primary text-primary-foreground shadow-md' : `${cat.color} border border-border/30 hover:opacity-80`}`}>
            ...
            <Card className="glass-card overflow-hidden group cursor-pointer">
              <div className="grid md:grid-cols-2 gap-0">
                ...
                <CardContent className="p-8 flex flex-col justify-center">
                  <span className="inline-flex w-fit items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold mb-4">Featured Article</span>
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6">
                    ...
                    <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> {featured.readTime} read</span>
                  </div>
                  <Button variant="gradient" className="w-fit rounded-full gap-2">
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    {BLOG_CATEGORIES.find(c => c.id === post.category) && (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${BLOG_CATEGORIES.find(c => c.id === post.category)?.color}`}>
                        {BLOG_CATEGORIES.find(c => c.id === post.category)?.icon} {BLOG_CATEGORIES.find(c => c.id === post.category)?.label}
                      </span>
                    )}
                  </div>
                </div>
                <CardContent className="p-5 flex flex-col flex-1">
                  <div className="text-xs text-primary font-bold mb-2">{post.date}</div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-border/30">
                    <span className="text-xs text-muted-foreground flex items-center gap-1"><User className="h-3 w-3" /> {post.author}</span>
