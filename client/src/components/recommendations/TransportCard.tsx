      ...
      <CardContent className="p-4 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <h3 className="text-lg font-bold text-foreground">
              {transport.type === 'flight' ? transport.vehicleType : transport.provider}
            </h3>
        ...
        <div className="flex items-center justify-between text-sm my-2 px-2 py-3 bg-background/50 rounded-lg border border-border/50">
          <div className="text-center">
            <p className="font-bold text-lg">{new Date(transport.departureTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
            <p className="text-xs font-semibold text-muted-foreground bg-muted px-2 py-0.5 mt-1 rounded-md">{transport.origin}</p>
          </div>
          <div className="flex flex-col items-center px-4 text-muted-foreground">
            ...
            {transport.type === 'flight' && (
               <span className="text-[9px] mt-1 text-muted-foreground">
                 {(transport as any).stops === 0 ? 'Non-stop' : `${(transport as any).stops} stops`}
               </span>
            )}
          </div>
          <div className="text-center">
            <p className="font-bold text-lg">{new Date(transport.arrivalTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
            <p className="text-xs font-semibold text-muted-foreground bg-muted px-2 py-0.5 mt-1 rounded-md">{transport.destination}</p>
          </div>
        </div>

        <div className="mt-auto pt-3">
          {transport.bookingUrl || transport.provider ? (
            <Button variant="outline" className="w-full font-bold shadow-sm" onClick={() => transport.bookingUrl ? window.open(transport.bookingUrl, '_blank') : setShowModal(true)}>
              Book on {transport.provider}
            </Button>
          ) : (
            <Button variant="outline" className="w-full font-bold shadow-sm" onClick={() => setShowModal(true)}>
              Select Option
            </Button>
          )}
        </div>
