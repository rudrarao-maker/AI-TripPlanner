...
export function SplitBillCalculator() {
  ...
  const handleAddMember = () => {
    if (newMember.trim()) {
      setMembers([...members, { id: Date.now().toString(), name: newMember, paid: 0 }]);
      setNewMember('');
    }
  };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2 pt-2">
            ...
            />
            <Button variant="outline" onClick={handleAddMember}><Plus className="h-4 w-4" /></Button>
          </div>
        </div>

        {/* Summary Info */}
        <div className="flex justify-between items-center p-4 bg-primary/10 rounded-xl text-sm font-medium">
          <span>Total Group Spend: <span className="font-bold text-primary">₹{totalSpent}</span></span>
          <span>Per Person: <span className="font-bold text-primary">₹{splitAmount.toFixed(0)}</span></span>
        </div>

        {/* Settlements */}
        <div>
          <h4 className="font-bold mb-3 text-sm uppercase tracking-wider text-muted-foreground">How to settle up</h4>
          {transactions.length === 0 ? (
            <p className="text-sm text-center text-muted-foreground p-4 bg-muted/20 rounded-lg">Everyone is settled up! 🎉</p>
          ) : (
            <div className="space-y-2">
              {transactions.map((tx, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-card">
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-semibold text-destructive">{tx.from}</span>
                    <ArrowRight className="h-4 w-4 text-muted-foreground" />
                    <span className="font-semibold text-green-500">{tx.to}</span>
                  </div>
