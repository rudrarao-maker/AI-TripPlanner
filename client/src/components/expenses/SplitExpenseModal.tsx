...
interface SplitExpenseModalProps {
  ...
  onSave: (splits: { userId: string; amount: number }[], type: 'equal' | 'exact' | 'percentage') => void;
}

export function SplitExpenseModal({ isOpen, onClose, totalAmount, collaborators, onSave }: SplitExpenseModalProps) {
  const [splitType, setSplitType] = useState<'equal' | 'exact' | 'percentage'>('equal');
  const [selectedUsers, setSelectedUsers] = useState<Set<string>>(new Set(collaborators.map(c => c.id)));
  ...
  const calculateSplit = () => {
    ...
    if (splitType === 'equal') {
      ...
      selectedUsers.forEach(userId => {
        splits.push({ userId, amount: parseFloat(exactAmounts[userId] || '0') });
      });
    } else if (splitType === 'percentage') {
      selectedUsers.forEach(userId => {
        const percent = parseFloat(percentages[userId] || '0');
        splits.push({ userId, amount: parseFloat(((totalAmount * percent) / 100).toFixed(2)) });
      });
    }
  ...
  const handleSave = () => {
    ...
    if (splitType === 'percentage') {
      const sum = selectedUsers.size > 0 
        ? Array.from(selectedUsers).reduce((acc, userId) => acc + parseFloat(percentages[userId] || '0'), 0)
        : 0;
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
                {collaborators.map(user => (
                  <div key={user.id} className="flex items-center justify-between p-3 rounded-xl border border-border/50 hover:bg-muted/30 transition-colors">
                    <div className="flex items-center gap-3 cursor-pointer" onClick={() => toggleUser(user.id)}>
                      <div className={`h-5 w-5 rounded-full border flex items-center justify-center ${selectedUsers.has(user.id) ? 'bg-primary border-primary' : 'border-muted-foreground'}`}>
                      ...
                      <div className="h-8 w-8 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold">
                        {user.avatar ? <img src={user.avatar} className="rounded-full" /> : user.name[0]}
                      </div>
                      ...
                      <div className="w-24">
                        ...
                        {splitType === 'exact' && (
                          <Input 
                            ...
                            onChange={(e) => setExactAmounts(prev => ({...prev, [user.id]: e.target.value}))}
                          />
                        )}
