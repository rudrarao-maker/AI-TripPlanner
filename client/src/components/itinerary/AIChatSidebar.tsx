...
import { Send, Bot, User, Sparkles, X, ChevronRight, Zap, RefreshCw, Mic } from 'lucide-react';
...
export function AIChatSidebar({ isOpen, onClose, tripContext }: AIChatSidebarProps) {
          ...
          >
            ...
            <div className="p-4 border-b flex items-center justify-between bg-card">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-foreground leading-tight">TripCraft AI</h3>
                  <div className="flex items-center gap-1.5">
                    <span className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse" />
                    <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">Online • Gemini Powered</p>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon" onClick={clearChat} className="rounded-full h-8 w-8" title="Clear chat">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
              {messages.map((msg, idx) => (
                ...
                >
                  <div className={`flex max-w-[88%] gap-2 ${msg.type === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`shrink-0 h-7 w-7 rounded-full flex items-center justify-center mt-auto ${
                      msg.type === 'user'
                        ? 'bg-primary text-primary-foreground'
                        : 'bg-gradient-to-br from-primary/20 to-accent/20 text-primary'
                    }`}>
                      {msg.type === 'user' ? <User className="h-3.5 w-3.5" /> : <Bot className="h-3.5 w-3.5" />}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {/* Timestamp */}
                      <p className={`text-[10px] text-muted-foreground/50 ${msg.type === 'user' ? 'text-right' : ''}`}>
