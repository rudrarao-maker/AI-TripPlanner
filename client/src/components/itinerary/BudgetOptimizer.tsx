...
import { Wallet, TrendingDown, TrendingUp, AlertCircle, Sparkles } from 'lucide-react';
import { Button } from '@/components/ui/button';

export function BudgetOptimizer({ budget, itineraryDays = [] }: { budget: number; itineraryDays?: any[] }) {
      ...
      <CardContent className="p-5 space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-foreground flex items-center gap-2">🏨 Accommodation</span>
            <span className="font-medium">₹{totalHotelCost.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-foreground flex items-center gap-2">🎯 Activities & Food</span>
            <span className="font-medium">₹{totalActivityCost.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-foreground flex items-center gap-2">✈️ Transport (Est.)</span>
            <span className="font-medium">₹15,000</span>
          </div>
        </div>

        {/* AI Insight */}
        <div className={`p-4 rounded-xl flex items-start gap-3 ${isOverBudget ? 'bg-red-500/10 text-red-700 dark:text-red-400' : 'bg-green-500/10 text-green-700 dark:text-green-400'}`}>
          {isOverBudget ? <AlertCircle className="h-5 w-5 shrink-0 mt-0.5" /> : <Sparkles className="h-5 w-5 shrink-0 mt-0.5" />}
          <div>
            <p className="font-semibold text-sm">
              {isOverBudget ? 'You are over budget!' : 'Great! You are under budget.'}
            </p>
        ...
        {isOverBudget && (
          <Button variant="outline" className="w-full text-primary hover:text-primary border-primary/20 hover:bg-primary/10">
