...
import { IndianRupee, Plus, Receipt, User, ArrowRight, Wallet, Check, X, Users } from 'lucide-react';
...
export function ExpenseTracker({ tripId, collaborators }: { tripId: string, collaborators: Collaborator[] }) {
  ...
  const allUsers: SplitUser[] = useMemo(() => {
    ...
    if (user) {
      list.push({ id: user.id, name: user.name || 'Me', avatar: user.avatar });
    }
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      setIsAdding(false);
      setNewExpense({ description: '', amount: '', category: 'Food', splitType: 'equal', paidBy: user?.id || '' });
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-6">
          {transactions.length === 0 ? (
            <div className="text-center py-6">
              ...
              <p className="text-muted-foreground mt-1">Add a shared expense to see balances here.</p>
            </div>
          ) : (
            <div className="space-y-4">
              <h4 className="font-medium text-sm text-muted-foreground uppercase tracking-wider mb-2">Who owes who</h4>
                  ...
                  <div key={idx} className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border/50">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2 font-medium">
                        <span className="text-foreground">{getUserName(t.from)}</span>
                        <ArrowRight className="h-4 w-4 text-muted-foreground" />
                        <span className="text-foreground">{getUserName(t.to)}</span>
                      </div>
                    </div>
                    <div className="font-bold text-lg text-rose-500 flex items-center gap-1">
                      <IndianRupee className="h-4 w-4" /> {t.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                  </div>
                ))}
              </div>
          ...
          >
            <Card className="border-primary shadow-lg border-2">
              <CardHeader className="pb-4 flex flex-row items-center justify-between">
                <CardTitle className="text-lg">Add New Shared Expense</CardTitle>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      onChange={e => setNewExpense(prev => ({...prev, splitType: e.target.value}))}
                    >
                ...
                <div className="flex justify-end pt-2">
                  <Button variant="gradient" onClick={handleAddExpense} className="w-full md:w-auto">
                    Save Shared Expense
                  </Button>
                </div>
              </CardContent>
      ...
      {isLoading ? (
        <div className="h-32 flex items-center justify-center text-muted-foreground">Loading expenses...</div>
      ) : expenses.length === 0 ? (
        ...
        <div className="grid gap-3">
          {expenses.map((expense) => (
            <div key={expense.id} className="flex justify-between items-center p-4 bg-card rounded-xl border hover:shadow-md transition-shadow">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Paid by <span className="font-medium text-foreground">{expense.user?.name || 'Someone'}</span> on {formatDate(expense.date)}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-bold text-lg flex items-center justify-end gap-1">
                  <IndianRupee className="h-4 w-4" /> {expense.amount.toLocaleString()}
                </p>
