...
const DESTINATIONS = [
  ...
  },
  {
    ...
    description: 'Iconic white-washed buildings and stunning Mediterranean sunsets.',
    ...
    <div className="space-y-6 animate-fade-in mt-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Trending destinations recommended just for you.</p>
        </div>
      </div>
            ...
            <Card className="overflow-hidden border-none shadow-lg h-[280px] relative cursor-pointer">
              ...
              <CardContent className="absolute inset-0 p-6 flex flex-col justify-end z-10 text-white">
                <div className="flex gap-2 mb-3">
                  {dest.tags.map(tag => (
                    <span key={tag} className="px-2.5 py-1 rounded-full bg-white/20 backdrop-blur-md text-[10px] uppercase font-bold tracking-wider">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="translate-y-8 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                  <Button variant="gradient" size="sm" className="w-full gap-2 shadow-xl shadow-primary/25" asChild>
                    <Link to={`/plan?destination=${encodeURIComponent(dest.name)}`}>
