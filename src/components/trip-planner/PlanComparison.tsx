          ...
          {plans.map((plan: any, idx: number) => {
            ...
            const totalHotelCost =
              plan.days?.reduce((sum: number, d: any) => {
                const hotelActivities = d.activities?.filter((a: any) => a.category === "hotel") || [];
                return sum + hotelActivities.reduce((acc: number, a: any) => acc + (Number(a.estimatedCost) || 0), 0);
              }, 0) || 0;
              
            const totalActivityCost =
              plan.days?.reduce((sum: number, d: any) => {
                const nonHotelActivities = d.activities?.filter((a: any) => a.category !== "hotel") || [];
                return sum + nonHotelActivities.reduce((acc: number, a: any) => acc + (Number(a.estimatedCost) || 0), 0);
              }, 0) || 0;
              
            const isRecommended = idx === 1;
            
            const firstHotelActivity = plan.days?.[0]?.activities?.find((a: any) => a.category === "hotel");
            const hotelName = firstHotelActivity?.name || (firstHotelActivity?.location ? firstHotelActivity.location : "Hotel");
