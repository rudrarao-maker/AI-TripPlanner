...
async function fetchRealWeather(
  ...
  for (const [dateStr, items] of dailyMap) {
    ...
    const conditionCount = conditions.reduce(
    ...
    const description =
      items.find((i: any) => i.weather[0].main === mainCondition)?.weather[0]
...
function generateMockWeather(
  ...
  const current: CurrentWeather = {
    ...
    rainChance,
    cloudCover:
      condition === "Clear" ? 10 : 50 + Math.floor(Math.random() * 40),
    sunrise: "06:15 AM",
    sunset: "06:45 PM",
    updatedAt: new Date().toISOString(),
  };
