...
export const generateItinerary = async (tripDetails: any) => {
        ...
        activities: [
          {
            ...
            category: "string (beach, restaurant, museum, transport, hotel)",
          },
        ],
      },
    ],
  ...
  const destKey =
    Object.keys(DESTINATION_DATA).find((k) => dest.includes(k)) || "";
  ...
  if (dData) {
    ...
    const filtered =
      matchTypes.length > 0
        ? dData.places.filter((p) => matchTypes.includes(p.type))
        : dData.places;
    placesPool = (filtered.length >= 3 ? filtered : dData.places).map((p) => ({
      ...
      location: p.location,
      costMul:
        p.type === "adventure" ? 0.15 : p.type === "relaxation" ? 0.08 : 0.05,
    }));

    const foodFiltered = foodPref.includes("veg")
      ? dData.foods.filter(
          (f) =>
            f.name.toLowerCase().includes("veg") ||
            f.desc.toLowerCase().includes("plant"),
        )
      : dData.foods;
    foodsPool = (foodFiltered.length > 0 ? foodFiltered : dData.foods).map(
      (f) => ({
        title: `🍽️ ${f.name}`,
        desc: `${f.desc} 📍 ${f.spot}`,
        location: f.spot,
    ...
    placesPool = [
      {
        title: `Explore ${tripDetails.destination} Old Town`,
        desc: "Walk through the historic quarter, visiting local landmarks, street art, and hidden courtyards.",
        location: "City Center",
        costMul: 0.05,
      },
      {
        title: `${tripDetails.destination} Scenic Viewpoint`,
        desc: "Panoramic views of the entire region. Best visited at golden hour for photographs.",
        location: "Hilltop",
        costMul: 0.03,
      },
      {
        title: "Local Art & Craft Market",
        desc: "Shop for handmade souvenirs, local spices, textiles, and artisan pottery.",
        location: "Market Square",
        costMul: 0.05,
      },
      {
        title: `Nature Trail & Wildlife Walk`,
        desc: "Guided 2-hour walk through local flora and fauna with expert naturalist.",
        location: "Nature Reserve",
        costMul: 0.08,
      },
      {
        title: `${tripDetails.destination} Heritage Museum`,
        desc: "Learn about the region's fascinating history, architecture, and cultural evolution.",
        location: "Museum District",
        costMul: 0.04,
      },
      {
        title: "Sunset Boat Cruise",
        desc: "Relaxing 90-minute cruise with snacks and live music as the sun sets.",
        location: "Waterfront",
        costMul: 0.12,
      },
    ];
    foodsPool = [
      {
        title: "🍽️ Local Thali Experience",
        desc: "A grand platter of regional specialties — curries, breads, rice, and dessert.",
        location: "Food Street",
        costMul: 0.04,
      },
      {
        title: "🍽️ Street Food Walking Tour",
        desc: "Guided tour sampling 6+ iconic street food stalls with a local foodie.",
        location: "Old Town",
        costMul: 0.05,
      },
      {
        title: "🍽️ Rooftop Fine Dining",
        desc: "Multi-course dinner with panoramic views, craft cocktails, and live acoustic music.",
        location: "City Center",
        costMul: 0.08,
      },
    ];
  ...
  for (let i = 1; i <= diffDays; i++) {
    ...
    days.push({
      ...
      activities: [
        {
          time: "09:00 AM",
          name: morningPlace.title,
          description: morningPlace.desc,
          location: morningPlace.location,
          estimatedCost: Math.round(dailyBudget * morningPlace.costMul),
          duration: 180,
          category: morningPlace.title.toLowerCase().includes("beach")
            ? "beach"
            : "museum",
        },
        {
          time: "02:00 PM",
          name: afternoonPlace.title,
          description: afternoonPlace.desc,
          location: afternoonPlace.location,
          estimatedCost: Math.round(dailyBudget * afternoonPlace.costMul),
          duration: 150,
        ...
        },
        {
          ...
          estimatedCost: Math.round(dailyBudget * food.costMul),
          duration: 90,
          category: "restaurant",
        },
        {
          time: "10:00 PM",
          name: hotelName,
          description: `Overnight stay (${hotelCat.includes("luxury") ? 4.9 : hotelCat.includes("budget") ? 3.8 : 4.5} star)`,
          location: tripDetails.destination,
          estimatedCost: Math.round(
            dailyBudget *
              (hotelCat.includes("luxury")
                ? 0.4
                : hotelCat.includes("budget")
                  ? 0.15
                  : 0.3),
          ),
          duration: 600,
