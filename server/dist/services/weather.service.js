...
function generateAlerts(current, forecast) {
    ...
    if (current.temperature > 38) {
        alerts.push({
            ...
            description: `Temperature is ${current.temperature}°C. Stay hydrated and avoid prolonged sun exposure.`,
        });
    }
        ...
        alerts.push({
            severity: "warning",
            title: "Heat Advisory",
            description: `High temperature of ${current.temperature}°C expected. Carry water and sunscreen.`,
        });
    }
    if (current.uvIndex >= 8) {
        alerts.push({
            severity: "warning",
            title: "High UV Index",
            description: `UV Index is ${current.uvIndex}. Apply SPF 50+ sunscreen and wear protective clothing.`,
        });
    }
    ...
    if (rainyDays.length >= 3) {
        alerts.push({
            severity: "info",
            title: "Rain Expected",
            description: `Rain is likely on ${rainyDays.length} of the next ${forecast.length} days. Pack an umbrella and waterproof gear.`,
        });
    }
    if (current.windSpeed > 40) {
        alerts.push({
            ...
            description: `Wind speeds of ${current.windSpeed} km/h. Outdoor activities may be affected.`,
        });
    }
    if (current.temperature < 5) {
        alerts.push({
            ...
            description: `Temperature is ${current.temperature}°C. Bundle up with warm layers.`,
        });
    }
    return alerts;
}
...
async function fetchRealWeather(lat, lng) {
    ...
    for (const [dateStr, items] of dailyMap) {
        ...
        const conditionCount = conditions.reduce((acc, c) => {
            acc[c] = (acc[c] || 0) + 1;
            return acc;
        }, {});
        const mainCondition = Object.entries(conditionCount).sort(([, a], [, b]) => b - a)[0][0];
        const description = items.find((i) => i.weather[0].main === mainCondition)?.weather[0]
        ...
        forecast.push({
            ...
            humidity: Math.round(humidities.reduce((a, b) => a + b, 0) /
                humidities.length),
...
function generateMockWeather(lat, lng, locationName) {
    // Estimate climate zone from latitude
    const absLat = Math.abs(lat);
    const month = new Date().getMonth(); // 0-11
    const isSummer = (lat >= 0 && month >= 4 && month <= 8) ||
        (lat < 0 && (month <= 2 || month >= 10));
...
// ===== Exported functions =====
async function getCurrentWeather(lat, lng, locationName) {
