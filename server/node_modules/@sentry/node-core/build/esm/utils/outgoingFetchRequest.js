import { getClient, shouldPropagateTraceForUrl, getTraceData, mergeBaggageHeaders, getBreadcrumbLogLevelFromHttpStatusCode, addBreadcrumb, debug, parseUrl, getSanitizedUrlString, spanIsIgnored, getActiveSpan, withActiveSpan } from '@sentry/core';
...
function addTracePropagationHeadersToFetchRequest(request, propagationDecisionMap, span) {
  ...
  const addedHeaders = spanForTraceHeaders ? withActiveSpan(spanForTraceHeaders, () => getTraceData({ propagateTraceparent })) : getTraceData({ propagateTraceparent });
  if (!addedHeaders) {
    return;
  }
