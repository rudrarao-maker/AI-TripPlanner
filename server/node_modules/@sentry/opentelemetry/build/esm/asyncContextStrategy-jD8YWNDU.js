...
import { isObjectLike, parseUrl, getSanitizedUrlString, SDK_VERSION, derefWeakRef, addNonEnumerableProperty, makeWeakRef, isSentryRequestUrl, getClient, getRootSpan, spanIsIgnored, hasSpansEnabled, spanIsSentrySpan, spanIsSampled, baggageHeaderToDynamicSamplingContext, SEMANTIC_ATTRIBUTE_SENTRY_OP, SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, spanToJSON, SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME, stripUrlQueryAndFragment, dynamicSamplingContextToSentryBaggageHeader, LRUMap, debug, shouldPropagateTraceForUrl, parseBaggageHeader, SENTRY_BAGGAGE_KEY_PREFIX, generateSentryTraceHeader, generateTraceparentHeader, getDynamicSamplingContextFromSpan, getCurrentScope, getDynamicSamplingContextFromScope, getIsolationScope, propagationContextFromHeaders, shouldContinueTrace, spanToTraceContext, getTraceContextFromScope, generateTraceId, generateSpanId, _INTERNAL_safeMathRandom, handleCallbackErrors, getSpanStatusFromHttpCode, SPAN_STATUS_ERROR, SPAN_STATUS_OK, _INTERNAL_safeDateNow, debounce, timedEventsToMeasurements, captureEvent, getCapturedScopesOnSpan, SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, convertSpanLinksForEnvelope, getStatusMessage, spanTimeInputToSeconds, SPAN_KIND, safeSetSpanJSONAttributes, spanKindToName, hasSpanStreamingEnabled, addChildSpanToSpan, getDefaultIsolationScope, getDefaultCurrentScope, setCapturedScopesOnSpan, logSpanStart, logSpanEnd, shouldIgnoreSpan, parseSampleRate, sampleSpan, spanShouldInferOtelSource, spanSourceWasExplicitlySet, markSpanAsTracerProviderSpan, markSpanForOtelSourceInference, _INTERNAL_setSpanForScope, startNewTrace as startNewTrace$1, _INTERNAL_startInactiveSpan, withScope, SentryNonRecordingSpan, setAsyncContextStrategy } from '@sentry/core';
import { URL_FULL, HTTP_URL, HTTP_REQUEST_METHOD, HTTP_METHOD, DB_SYSTEM_NAME, DB_SYSTEM, RPC_SERVICE, MESSAGING_SYSTEM, FAAS_TRIGGER, DB_STATEMENT, HTTP_TARGET, HTTP_ROUTE, HTTP_RESPONSE_STATUS_CODE, HTTP_STATUS_CODE, RPC_GRPC_STATUS_CODE, SENTRY_ORIGIN, SERVICE_VERSION, SERVICE_NAME } from '@sentry/conventions/attributes';
...
function applyOtelSpanData(span, options = {}) {
  ...
  const attributesForInference = canInferSource && attributes[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] === "custom" ? { ...attributes, [SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: void 0 } : attributes;
  const inferred = inferSpanData(spanJSON.description || "<unknown>", attributesForInference, kind);
  if (kind !== SpanKind.INTERNAL && attributes["otel.kind"] === void 0) {
    span.setAttribute("otel.kind", SpanKind[kind]);
  }
  if (inferred.op && attributes[SEMANTIC_ATTRIBUTE_SENTRY_OP] === void 0) {
    span.setAttribute(SEMANTIC_ATTRIBUTE_SENTRY_OP, inferred.op);
  }
  const shouldApplyInferredSource = inferred.source !== void 0 && inferred.source !== "custom" && (options.finalizeStatus || inferred.source !== "url") && (spanJSON.parent_span_id === void 0 || kind === SpanKind.SERVER);
  if (shouldApplyInferredSource && (attributes[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] === void 0 || canInferSource)) {
  ...
  if (mayInferSource && inferred.description !== spanJSON.description && (attributes[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] !== "custom" || canInferSource)) {
    span.updateName(inferred.description);
  }
}
function applyOtelSpanKind(span, kind) {
  addNonEnumerableProperty(span, "kind", kind ?? SpanKind.INTERNAL);
}
...
class SentryTracer {
  ...
  startSpan(name, options = {}, ctx) {
    ...
    const parentSpan = options.root ? void 0 : trace.getSpan(parentContext);
    if (isTracingSuppressed$1(parentContext)) {
    ...
    if (options.attributes?.[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] === void 0) {
  ...
  startActiveSpan(name, optionsOrFn, contextOrFn, fn) {
    ...
    const ctx = typeof contextOrFn === "function" || contextOrFn === void 0 ? context.active() : contextOrFn;
  ...
  _startSentrySpan(name, options, parentSpan, hasExplicitContext) {
    ...
    if (options.root) {
      return startNewTrace$1(() => _INTERNAL_startInactiveSpan({ ...sentryOptions, parentSpan: null }));
    }
    if (parentSpan?.spanContext().isRemote) {
      return this._startRootSpanWithRemoteParent(sentryOptions, parentSpan);
    }
    if (parentSpan) {
      return _INTERNAL_startInactiveSpan({ ...sentryOptions, parentSpan });
    }
    return startNewTrace$1(
  ...
  _startRootSpanWithRemoteParent(options, parentSpan) {
    ...
    return withScope((scope) => {
      ...
      _INTERNAL_setSpanForScope(scope, void 0);
...
export { startSpan as A, startSpanManual as B, suppressTracing as C, withActiveSpan as D, wrapClientClass as E, wrapContextManagerClass as F, wrapSamplingDecision as G, buildContextWithSentryScopes as H, SENTRY_SCOPES_CONTEXT_KEY as I, SENTRY_TRACE_STATE_CHILD_IGNORED as J, SEMANTIC_ATTRIBUTE_SENTRY_GRAPHQL_OPERATION as S, SentryPropagator as a, SentrySampler as b, SentrySpanProcessor as c, SentryTracerProvider as d, applyOtelSpanData as e, backfillStreamedSpanDataFromOtel as f, continueTrace as g, enhanceDscWithOpenTelemetryRootSpanName as h, getActiveSpan as i, getRequestSpanData as j, getScopesFromContext as k, getSentryResource as l, getSpanKind as m, getTraceContextForScope as n, isSentryRequestSpan as o, openTelemetrySetupCheck as p, setOpenTelemetryContextAsyncContextStrategy as q, setupEventContextTrace as r, setIsSetup as s, spanHasAttributes as t, spanHasEvents as u, spanHasKind as v, spanHasName as w, spanHasParentId as x, spanHasStatus as y, startInactiveSpan as z };
//# sourceMappingURL=asyncContextStrategy-jD8YWNDU.js.map
