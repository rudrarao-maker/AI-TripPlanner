...
function applyOtelSpanData(span, options = {}) {
  ...
  const attributesForInference = canInferSource && attributes[core.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] === "custom" ? { ...attributes, [core.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: void 0 } : attributes;
...
function applyOtelSpanStatus(span, attributes, status, spanStreamingEnabled) {
  if (status === void 0) {
    span.setStatus(inferStatusFromAttributes(attributes) || { code: core.SPAN_STATUS_OK });
    return;
  }
...
class SentryTracer {
  ...
  _startSentrySpan(name, options, parentSpan, hasExplicitContext) {
    ...
    if (options.root) {
      return core.startNewTrace(() => core._INTERNAL_startInactiveSpan({ ...sentryOptions, parentSpan: null }));
    }
    if (parentSpan?.spanContext().isRemote) {
      return this._startRootSpanWithRemoteParent(sentryOptions, parentSpan);
    }
    if (parentSpan) {
      return core._INTERNAL_startInactiveSpan({ ...sentryOptions, parentSpan });
    }
    return core.startNewTrace(
      () => core._INTERNAL_startInactiveSpan({
        ...sentryOptions,
  ...
  _startRootSpanWithRemoteParent(options, parentSpan) {
    ...
    return core.withScope((scope) => {
      ...
      return core._INTERNAL_startInactiveSpan({ ...options, parentSpan: null });
    });
  }
  _createNonRecordingSpan(parentSpan) {
    const span = new core.SentryNonRecordingSpan({ traceId: parentSpan?.spanContext().traceId });
    if (parentSpan) {
      core.addChildSpanToSpan(parentSpan, span);
    }
...
//# sourceMappingURL=asyncContextStrategy-volGaYqZ.js.map
